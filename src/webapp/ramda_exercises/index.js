...
const reversedRange = R.compose(R.reverse, R.range);
...
const createMoviesElements = R.compose(R.map(createElement),
...
const createGenresTemplate = R.compose(R.join(''), R.map(genre => `<li>${genre.name}</li>`));
...
onClick('.btn-favorite', function() {
    addMovieToFavorites(userMoviesService, getMovieId(this), getDataAttrValue('movie-title', this));
